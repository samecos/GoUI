...
未设置环境变量时，前端连接**浏览器地址栏中的主机名**、端口 `8090`、路径 `/ws`：

| 打开的前端地址 | 默认 WebSocket 地址 |
| --- | --- |
| `http://localhost:5173` | `ws://localhost:8090/ws` |
| `http://192.168.1.10:5173` | `ws://192.168.1.10:8090/ws` |
| `https://go.example.com` | `wss://go.example.com:8090/ws` |
...
- 候选点悬停 300 毫秒后读取已有搜索图中的后续变化（PV），支持停一手与提子显示；悬停不会请求启动新的推理任务。
...
将生成的 `dist/` 内容部署到静态网站服务器。后端需要单独运行，并确保访问者的浏览器能够连接配置的 WebSocket 地址。如果通过反向代理接入后端，代理需要支持 WebSocket 升级。
