...
- 桌面工作台按窗口高度完整显示棋盘，分析栏可拖动调宽，支持方向键微调和双击恢复。专注模式隐藏分析栏，手机和平板将复盘控制放在棋盘下方。
- 棋盘采用本地 SVG 木纹、厚度和落子阴影；黑白棋子使用独立光照渐变与白子细纹，手数及候选点仍可独立切换。
- 分析详情可切换推荐选点、胜率走势和落子记录；手数滑条支持快速定位，分析栏宽度保存在当前浏览器会话中。
- 触屏分析采用两次点按：第一次点选位置查看已有变化，松手后保留预览；再次点同一位置才落子，点其他位置切换预览，可在棋盘下方退出。没有已有变化的点也先选中、再次点按确认。自由对弈仍直接点按落子。支持 Pointer Events 和旧版 Touch Events；滑动、双指缩放与取消手势不会落子。
- 19 路中国规则：双方手动落子、停一手、悔棋、清枰、重开与设置贴目。棋局合法性和最终状态以服务端快照为准。
- 逐手复盘、自动播放和历史定位；未导入 SGF 的普通棋局在历史位置落子时，兼容服务会截断后续主线。
- 开始／停止持续分析、AI 落子与取消。AI 落子按服务预算执行并直接落子，无算力时显示服务错误。
- 「AI 分析 → 搜索参数」或「偏好设置」可启停 PDA 和宽根搜索，设置 PDA 数值（−3～3）、参照方及宽根强度（0～5，可从 0.04 开始）。首次开启 PDA 默认固定当前黑／白一方，使正常落子继承已搜索分支；显式选择“随行棋方”会在换方时重算。改变实际 PDA 需清图，仅宽根或等效固定参照变化保留已有搜索；分析卡片显示继承访问量、节点或重算原因。参数由 Server 会话保存，刷新及短暂重连恢复；须更新 Server 后使用。
- 展示黑方胜率、目差、候选点、搜索访问量和搜索速度。胜率曲线只记录本页面收到的局面评估，未分析的手数保留空缺。
- 鼠标悬停候选点 300 毫秒后读取已有搜索图中的后续变化（PV），触屏点选立即读取；预览期间每次请求完成后间隔 1 秒更新，相同变化不重绘。触屏预览不受候选排名刷新影响，退出、落子或切换局面时取消。支持停一手与提子显示；预览不会请求启动新的推理任务。
...
- SGF 导入成功后自动回到第 0 手空枰，保留完整主线。复盘时用橙色圆环标出棋谱下一手，并显示行棋方与坐标；停一手显示文字提示，末手自动隐藏提示。提示独立于 AI 候选点开关。
- 点中 SGF 下一手落点等同“下一步”；点其他位置进入试下，原谱保留。回退、悔棋、滑条定位或自动播放会返回原谱，导出也保留原谱。原谱与服务会话绑定并保存在当前浏览器会话中，刷新后仍可从试下返回；清枰、重开或服务会话过期会清除原谱。
...
[`browser_sgf_review.py`](scripts/browser_sgf_review.py) 使用同一浏览器环境和协议夹具，覆盖 SGF 空枰起步、点中下一手、试下后返回原谱、停一手、提示、自动播放、刷新恢复和导入失败。
