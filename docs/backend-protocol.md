...
构建时可通过 `VITE_ENGINE_WS_URL` 指定完整地址。未配置时连接当前页面主机和端口的 `/ws`：HTTP 页面使用 `ws://`，HTTPS 页面使用 `wss://`。Vite 开发和预览服务器将该路径代理至 `ws://127.0.0.1:8090`；生产部署需配置对应的 WebSocket 反向代理，或显式指定可访问的后端地址。前端使用浏览器原生 WebSocket，未指定子协议，也未实现登录或应用层鉴权流程。
...
SGF 导入成功后，前端继续发送 `seek {position:0}` 回到空枰，期间保持修改控件禁用；完整主线仍保留在 `moves` 中。若返回开局失败，界面保留服务最后确认的局面并提示重试，不会只在本地伪造第 0 手。

前端在当前浏览器会话中另外保存导入的原谱，并绑定服务会话。点击原谱下一手发送 `seek`；试下其他位置仍发送 `play`，服务可按原协议截断主线。之后使用复盘控件时，若服务主线与原谱不同，前端先用 `set_position` 恢复完整原谱，再 `seek` 到目标手数；两步完成前保持修改控件禁用。服务协议无须增加分支字段。
